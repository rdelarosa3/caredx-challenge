...
//----------------------------------------------------------
/**
 * Register this library as a Page API
 * @author Skuid
 */
window.skuid.runtime.registerApi("v2", "lib/odatav4", function(skuid){
//----------------------------------------------------------
...
function handlerAccepts(handler, cType) {
    //----------------------------------------------------------
    /**
     * Because Jitterbit returns a text/xml contenttype instead of
...
function isJsonSerializableProperty(attribute) {
    ...
    //----------------------------------------------------------
    /**
     * To successfully update reference fields, we must be able to serialize
     * attributes that contain @odata.bind. - JLC
     * @author Skuid
     */
...
function checkProperties(data, objectInfoType, baseURI, model, recognizeDates) {
    //----------------------------------------------------------
    /**
     * This function was causing our payloads to register as a failure when the payload was just fine
     * So am short circuiting for now. Never seemed to work for us correctly anyway. - JACKSON
     * @author Skuid
     */
    return;
    //----------------------------------------------------------
...
//----------------------------------------------------------
/**
 * The original library contained a very naive approach to binding the request function to the defaultHttpClient object.
 * While it appeared to work, it was actually just binding the function to the window object. In strict/ES6 mode,
 * however, "this" just ended up being undefined. I've implemented the bind correctly here. --- jdb
 * @author Skuid
 */

Object.assign(exports.defaultHttpClient, {
    ...
    request: function(request, success, error) {
        ...
        return result;
    }.bind(exports.defaultHttpClient)
});
...
//----------------------------------------------------------
/**
 * In strict mode, these variables need to be explicitly defined.
 * @author Skuid
 */
let DomStore, IndexedDBStore, MemoryStore;
//----------------------------------------------------------
...
//----------------------------------------------------------
/**
 * Attach the odatajs object to skuid.lib (so that we don't pollute the window)
 * @author Skuid
 */
skuid.lib.odatajs = {};
init.call(this,skuid.lib.odatajs,skuid.lib.odatajs,require);
});
//----------------------------------------------------------
